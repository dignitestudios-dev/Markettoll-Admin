      ...
      <div className="flex flex-col mt-5">
        <div className="-m-1.5 overflow-x-auto">
          <div className="p-1.5 min-w-full inline-block align-middle">
            {refferals?.referredUsers &&
            refferals?.referredUsers?.length > 0 ? (
              <h3 className="font-bold text-xl mb-2">Referred Users</h3>
            ) : (
              ""
            )}
            <div className="border border-gray-200 rounded-lg overflow-hidden">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="divide-y divide-gray-200">
                  {refferals?.referredUsers &&
                  refferals?.referredUsers?.length > 0 ? (
                    refferals?.referredUsers?.map((item, i) => (
                    ...
                    <tr>
                      ...
                      >
                        No referred users records found.
                      </td>
                    </tr>
                  )}
                </tbody>
          ...
          <div className="p-1.5 min-w-full inline-block align-middle">
            {refferals?.referredAffiliate &&
            refferals?.referredAffiliate?.length > 0 ? (
              <h3 className="font-bold text-xl mb-2">
                Referred Affiliate Users
              </h3>
            ) : (
              ""
            )}
            <div className="border border-gray-200 rounded-lg overflow-hidden">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="divide-y divide-gray-200">
                  {refferals?.referredAffiliate &&
                  refferals?.referredAffiliate?.length > 0 ? (
                    refferals?.referredAffiliate?.map((item, i) => (
                      <tr key={i} className="cursor-pointer">
                        <th
                          onClick={() =>
                            navigate(`/user/${item?._id}`, {
